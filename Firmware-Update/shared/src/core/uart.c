...
#define RING_BUFFER_SIZE (128) // round up in a power of 2, for max of ~10ms of latency
...
void uart_setup(void) {
    ...
    usart_set_mode(USART2, USART_MODE_TX_RX); // transmitting and receiving, can choose only one
...
uint8_t uart_read_byte(void){
    // in this case we only read one byte of data
    // we don't care wether the read is successful or not
    // it is the user responsibility to check in advance with uart_data_available()
    uint8_t byte = 0;
    (void)uart_read(&byte, 1);
    // when you have a function that returns a value and you want to ignore that value
    // it's best practice to add (void)
    return byte;
}
