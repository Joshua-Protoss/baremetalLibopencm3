...
const VECTOR_TABLE_SIZE                  = (0X01B0);  // 0x01B0 check it first, 0x00081ac before

//const FWINFO_ADRESS                      = (FLASH_BASE + BOOTLOADER_SIZE + VECTOR_TABLE_SIZE);
...
const crc8 = (data: Buffer | Array<number>) => {
  ...
  for (const byte of data) {
    ...
    //console.log(`0x${byte.toString(16).padStart(2, '0')} 0x${crc.toString(16).padStart(2, '0')}`);
  }
...
uart.on('data', data => {
  ...
  while (rxBuffer.length >= PACKET_LENGTH) {
    ...
    packets.push(packet);
    writePacket(Packet.ack);    //writePacket function will save packet into lastpacket
  }
});
...
const syncWithBootloader = async (timeout = DEFAULT_TIMEOUT) => {
  ...
  while (true) {
    ...
    if (packets.length > 0) {
      ...
      Logger.error('Sync sequence failed: wrong packet');
      process.exit(1);
    }

    if (timeWaited >= timeout) {
      Logger.error('Time out waiting for sync sequence');
      process.exit(1);
    }
  }
}
...
const main = async () => {
  ...
  Logger.info('Waiting for a few seconds for main app to be erased...');
  await delay(1000);
  Logger.info('Waiting for a few seconds for main app to be erased...');
  await delay(1000);
  Logger.info('Waiting for a few seconds for main app to be erased...');
  ...
  while (bytesWritten < fwLength) {
    ...
    const dataBytes = fwImage.slice(bytesWritten, bytesWritten + PACKET_DATA_BYTES); // iterating
...
main()
  ...
  .finally(async () => {
    await delay(500);
    uart.close()    // uart.close() might override other error, remove it while error checking
  });
