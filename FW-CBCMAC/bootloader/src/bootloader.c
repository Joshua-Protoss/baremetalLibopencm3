...
static void gpio_teardown(void) {
    // uart gpio setup, analog is the default mode
...
// With cast: ((uint8_t*)state)[i] gives you the i-th BYTE
// ((uint8_t*)state)[0]  // First byte
// ((uint8_t*)state)[4]  // Fifth byte (first byte of second column)
// ((uint8_t*)state)[15] // Last byte, ((uint8_t*)state)[i] type cast state(4bytes jumpt into 1 byte jump)
// state[0]                    // TYPE: uint8_t* (address)
//                             // USE: &state[0][0]
// ((uint8_t*)state)[0]        // TYPE: uint8_t (value)
//                             // USE: actual byte stored there
...
static bool validate_firmware_image(void) {
    ...
    // we need to pad blocks
    // the standard way : if you need to add 4 bytes, fill those 4 bytes with 0x04.
    // decryption : remove those 4 bytes if they are all 0x04
...
static void bootloading_failed(void){
        comms_create_sByte_packet(&temp_packet, BL_PACKET_NACK_DATA0);
...
static bool is_device_id_packet(const comms_packet_t* packet) {
    // device id packet expect 2 bytes
    if (packet->length != 2) {
        return false;
    }
...
static bool is_fw_length_packet(const comms_packet_t* packet) {
    // device id packet expect 5 bytes
    if (packet->length != 5) {
        return false;
    }
...
int main(void) {
    ...
    while (state != BL_State_Done) {
        if (state == BL_State_Sync) {
            if (uart_data_available()){
                ...
                if (is_match){
                    comms_create_sByte_packet(&temp_packet, BL_PACKET_SYNC_OBSERVED_DATA0);
                    comms_write(&temp_packet);
        ...
        switch(state){
             case  BL_State_WaitForUpdateReq: {
                if (comms_packets_available()){
                    ...
                    if (comms_is_sByte_packet(&temp_packet, BL_PACKET_FW_UPDATE_REQ_DATA0)) {
                        simple_timer_reset(&timer);
                        comms_create_sByte_packet(&temp_packet, BL_PACKET_FW_UPDATE_RES_DATA0);
                        comms_write(&temp_packet);
             ...
             case  BL_State_DeviceIDReq: {
                simple_timer_reset(&timer);
                comms_create_sByte_packet(&temp_packet, BL_PACKET_DEVICE_ID_REQ_DATA0);
                comms_write(&temp_packet);
             ...
             case  BL_State_FWLengthReq: {
                simple_timer_reset(&timer);
                comms_create_sByte_packet(&temp_packet, BL_PACKET_FW_LENGTH_REQ_DATA0);
                comms_write(&temp_packet);
    ...
    // TODO teardown before jump to main
    system_delay(150);
    uart_teardown();
    gpio_teardown();
    system_teardown();
