...
bool comms_is_sByte_packet(const comms_packet_t* packet, uint8_t byte){
    if (packet->length != 1) {
        return false;
    }
...
void comms_create_sByte_packet(comms_packet_t* packet, uint8_t byte){
...
void comms_setup(void){
    comms_create_sByte_packet(&retx_packet, PACKET_RETX_DATA0);
    comms_create_sByte_packet(&ack_packet, PACKET_ACK_DATA0);
}
